...
int main(void)
{
  ...
  MicroPrintf("dims->size: %d", input->dims->size);
  ...
  {
    input->data.f[i] = (float)(image2[i] / 255.0);
  }

  MicroPrintf("Do inference");
  uint32_t start_time = k_uptime_get_32();
  MicroPrintf("Inference time = %d", start_time);
  ...
  MicroPrintf("Inference time = %d", duration);
